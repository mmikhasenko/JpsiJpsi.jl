...
let
    ...
    for (i, name) in enumerate(notebooklist)
        ...
        _name_list = split(_name, '/')[end]
        modtime = Dates.unix2datetime(mtime(joinpath(@__DIR__, name)))
        content *= " $(i). [**`$(_name_list)`**]($(_name_html)) (modified on $(Date(modtime)))\n"
    end
...
header(; title="Spin and Parity in the vector-vector system", author="Misha Mikhasenko, Liupan An, Ronan McNulty")
