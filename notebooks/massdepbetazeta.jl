...
md"""
# Mass-dependent angular modulation

The notebook implements computation of the angular modulation for a model
that with several partial waves. The $\beta / \zeta$ modulations are plotted
for two configurations:

- **No Interf**: $|\text{continuum}|^2 + |\text{broadBW}|^2 + |\text{narrowBW}|^2$
- **With Interf**: $|\text{continuum} + \text{broadBW}|^2 + |\text{narrowBW}|^2$

"""

# ╔═╡ 64ae4bf0-5081-4d4d-868a-18cedbeb7a6b
theme(:wong2, colorbar=false, clim=(-1, 1),
    frame=:box, grid=false, minorticks=true,
    guidefontvalign=:top, guidefonthalign=:right,
    foreground_color_legend=nothing, lab="",
    xlim=(:auto, :auto))

# ╔═╡ 03544cee-b52a-4c7f-9dfa-93573d914799
md"""
### Four groups of quantum numbers
"""

# ╔═╡ f38e815e-e168-4634-af9d-8fc5642e2b83
begin
    @with_kw struct HelicityMatrix
        a::ComplexF64 = 0
        b::ComplexF64 = 0
        c::ComplexF64 = 0
        d::ComplexF64 = 0
    ...
    # 
    function HelicityMatrix(jp::String; kw...)
        j = Meta.parse(jp[1:1])::Int
        is⁺p = jp[2] == '+'
        # 
        is⁺ϵ = ((-1)^j * (2is⁺p - 1)) == 1
        is⁺s = iseven(j)
        # 
        HelicityMatrix(; kw..., is⁺s, is⁺ϵ)
    end
end

# ╔═╡ 1c273085-b6b1-43ce-953d-18acb1d4e39b
function amplitude(hm::HelicityMatrix)
    @unpack a, b, c, d, N = hm
    @unpack is⁺s, is⁺ϵ = hm
    s = 2 * is⁺s - 1
    ϵ = 2 * is⁺ϵ - 1
    # 
    H = [(1+s)*b/2 a (1+s*ϵ)*c/2
        s*a (1+ϵ)*(1+s)*d/4 s*ϵ*a
...
# ╔═╡ e06b4fb2-e57b-11ec-29c5-012882cc41c2
begin
    @with_kw struct Wave{T,P}
        jp::String
        couplings::P
        lineshape::T
    end
    Wave(jp::String; lineshape, kw...) =
        Wave(; jp,
            couplings=HelicityMatrix(jp; kw...),
            lineshape)
end;

# ╔═╡ 46b36fd8-e759-40d4-889a-7ec90354f35a
md"""
## Lineshape
"""
...
plot(layout=grid(1, 4), size=(800, 180),
    title=["Group I" "Group II" "Group III" "Group IV"],
    ticks=([1, 2, 3], [1, 0, -1]),
    [heatmap(c=:balance,
...
applylineshape(wave::Wave, m) =
    amplitude(wave.couplings) .* amplitude(wave.lineshape, m)

# ╔═╡ 72466e49-23af-4bef-9a72-6fa1206e206a
function sum2(M::Model, m)
    grouppedM = M |> @groupby(_.jp) .|> collect
    X = sum(grouppedM) do Wv
...
function intensity(M::Model, m::Float64)
    H² = sum2(M, m)
    return sum(H²) * ρ(m)
end

# ╔═╡ 7fe6850c-ce45-458a-9924-af3e198a4484
@recipe function f(M::Model, x1::Number, x2::Number; operator=intensity)
    @series begin
        (Base.Fix1(operator, M), x1, x2)
    end
    for i in 1:length(M)
        @series begin
            fillrange --> 0
            α --> 0.4
            (Base.Fix1(operator, M[i:i]), x1, x2)
        end
    end
    return nothing
end

# ╔═╡ 58cff776-9abf-4a0a-81ec-6e17da148672
function beta(M::Model, m::Float64)
    grouppedM = M |> @groupby(_.jp) .|> collect
    ϵb² = sum(grouppedM) do Wv
        ϵ = (Wv[1].couplings.is⁺ϵ * 2 - 1)
        b² = abs2(sum(w -> applylineshape(w, m)[1, 1], Wv))
        ϵ * b²
    end
    H² = sum2(M, m)
    return ϵb² / sum(H²) / 2
...
function zeta(M::Model, m::Float64)
    H² = sum2(M, m)
    a², b², c², d² = H²[1, 2], H²[1, 1], H²[1, 3], H²[2, 2]
    return (b² + c² - a² - d²) / sum(H²)
end
...
savefig(joinpath(@__DIR__, "..", "plots", "two_scenarios.pdf"))
...
manifest_format = "2.0"
project_hash = "08e7b1a08de1b3ff2b494b37bddcc102e8d0b2dc"
